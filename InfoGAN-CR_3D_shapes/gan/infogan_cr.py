...
class INFOGAN_CR(object):
    ...
    def export_as_tf_hub(self, checkpoint_dir, export_dir):
        """Exports the provided gan as a TFHub model.
        Args:
            checkpoint_dir: String with path where to load weights from.
            export_dir: String with path where to save the TFHub module to.
        """
        def module_fn(is_training):
            """Module function used for TFHub export."""
            real_image_pl = tf.placeholder(
                tf.float32, [None] + self.image_dims)
            _, codes, _ = \
                self.infoGANDiscriminator.build(
                    real_image_pl, train=is_training, sn_op=NO_OPS)
            hub.add_signature(
                    name="representation",
                    inputs={"images": real_image_pl},
                    outputs={"default": codes})
        tags_and_args = [
            ({"train"}, {"is_training": True}),
            (set(), {"is_training": False}),
            ]
        spec = hub.create_module_spec(module_fn, tags_and_args=tags_and_args)
        spec.export(export_dir, checkpoint_path=checkpoint_dir)
        ...
        for epoch_id in tqdm(range(self.epoch)):
            ...
            for batch_id in range(batch_num):
                ...
                if (batch_id + 1) % self.vis_freq == 0:
                    checkpoint_dir = os.path.join(
                                    self.checkpoint_dir,
                                    "global_id-{}/tf_checkpoints".format(global_id - 1))
            ...
            self.save(global_id - 1, checkpoint_dir=checkpoint_dir)
            hub_dir = os.path.join(
                            self.checkpoint_dir,
                            "global_id-{}/tfhub".format(global_id - 1))
            if not os.path.exists(hub_dir):
                os.makedirs(hub_dir)
            self.export_as_tf_hub(checkpoint_dir=checkpoint_dir, export_dir=hub_dir)


if __name__ == "__main__":
    from latent import UniformLatent, JointLatent
    from network import Decoder, InfoGANDiscriminator, CrDiscriminator
    from load_data import load_3Dshapes
    from metric import FactorVAEMetric

    parser = argparse.ArgumentParser()
    parser.add_argument("--gin_config", default=["base.gin"], nargs="+", help="List of paths to config files.")
    parser.add_argument("--gin_bindings", default=[], nargs="+", help="Newline separated list of Gin parameter bindings.")
    parser.add_argument("--dir_name", default="./test{exp_id}", help="Dir to save output files.")
    args = parser.parse_args()

    gin_config_files = args.gin_config
    ...
    latent_list.append(
        UniformLatent(
            in_dim=10, out_dim=10, low=-1.0, high=1.0, apply_reg=False))
    ...
    exp_id = len(os.listdir("."))
    dir_name = dir_name.format(exp_id=exp_id)
    checkpoint_dir = "./{}/checkpoint".format(dir_name)
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
    sample_dir = "./{}/sample".format(dir_name)
    if not os.path.exists(sample_dir):
        os.makedirs(sample_dir)
    time_path = "./{}/time.txt".format(dir_name)
    metric_path = "./{}/metric.csv".format(dir_name)
    config_path = "./{}/config.gin".format(dir_name)
